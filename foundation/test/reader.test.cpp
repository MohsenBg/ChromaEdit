...
protected:
  ...
  void TearDown() override { fs::remove_all(DIST_DIR); }
};

TEST_F(ReaderTest, FindImageFiles) {
  vector<fs::path> expected_image_paths{
    ReaderTest::DIST_DIR / "image.bmp", ReaderTest::DIST_DIR / "image.gif",
      ReaderTest::DIST_DIR / "image.jpg", ReaderTest::DIST_DIR / "image.png",
      ReaderTest::DIST_DIR / "image.tiff", ReaderTest::DIST_DIR / "image.webp",
  };
...
TEST_F(ReaderTest, GetFileType) {
  ...
  ASSERT_EQ("pdf",
    Reader::get_file_type(ReaderTest::DIST_DIR / "document.pdf"));
  ASSERT_EQ("zip", Reader::get_file_type(ReaderTest::DIST_DIR / "archive.zip"));
}

TEST_F(ReaderTest, GetFileName) {
  ASSERT_EQ("image", Reader::get_file_name(ReaderTest::DIST_DIR / "image.jpg"));
  ASSERT_EQ("image", Reader::get_file_name(ReaderTest::DIST_DIR / "image.png"));
  ASSERT_EQ("archive",
    Reader::get_file_name(ReaderTest::DIST_DIR / "archive.zip"));
  ASSERT_EQ("document",
    Reader::get_file_name(ReaderTest::DIST_DIR / "document.pdf"));
  ASSERT_EQ("", Reader::get_file_name(ReaderTest::DIST_DIR / "not_found.zip"));
}
